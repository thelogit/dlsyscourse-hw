...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # Compute gradient of each node in reverse topological order
    for node in reverse_topo_order:
        grad = sum_node_list(node_to_output_grads_list[node])
        node.grad = grad

        if not node.op or not node.inputs:
            continue

        partial_grads = node.op.gradient_as_tuple(out_grad=grad, node=node)
